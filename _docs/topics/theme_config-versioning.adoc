...
You can use version handlers however you want, but we present here some guidance for choosing among the options to suit your use case.
...
The handler types perform distinct actions on the selected JavaScript `ready`, `change`, `focus`, or similar event.
...
The rest of this topic provides an overview of attributes and functions common across handlers and to help you choose between the various types of version handlers as well as .
...
Most common with code listings and log samples, [.case]*_if you need to alternate versions of content inside a fixed block_* without forcing the user to scroll, your solution is *tabbing*.

These handlers JavaScript to glean combinations of selector *tabs* and content *panes* from the HTML rendered from conventional AsciiDoc syntax.
...
There is no need to register them in a data file, though some options are available for doing so.
...
A *toggle* lets you show one class while hiding the rest of the classes in a group.
...
Drop something like `+++[.tok.vrsn]*0.2.0*+++` into your page, and watch the version change as you switch between versions using the buttons to the right, below the TOC: [.tok.vrsn]*0.2.0*.
...
This feature could be invaluable to the process of "`white labeling`" a docset.
...
They are like an open-ended token swapper, targeting any discrete aspect of an HTML element that you can latch onto with jQuery.
...
All handlers plant a cookie on the visiting browser so their choice is recalled for subsequent URLs during the session or until they clear their cookies.
