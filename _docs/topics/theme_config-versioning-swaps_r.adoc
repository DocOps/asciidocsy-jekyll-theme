...
Token swaps enable you to place variables in your AsciiDoc or other page elements that can be altered either by an act of the user or a URL query string.
...
A swap can be triggered by a `selectbox` dropdown or radio `buttons`.
...
hard token::
As an alternative that is easy to store in an AsciiDoc attribute (see <<store in an AsciiDoc attribut>> below), use a token formatted like so: `<$tok.token_name>`.
...
features:
  actions:
    versioning:
      ...
      patch-swap-0_2:
include::{path_to_jekyll_config}[tags="sample-swap-patch"]
----
...
features:
  actions:
    versioning:
      ...
      theme-name-tokens:
include::{path_to_jekyll_config}[tags="sample-swap-spoof"]
----
